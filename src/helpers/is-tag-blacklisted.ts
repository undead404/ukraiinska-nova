...
const blacklistedTagsData = await readFile('./blacklisted-tags.txt');

const BLACKLISTED_TAGS = blacklistedTagsData.toString().split('\n');
...
export default function isTagBlacklisted(tagName: string): boolean {
  ...
  if (tagName.length < MIN_TAG_NAME_LENGTH) {
    ...
    result = true;
  } else if (BLACKLISTED_TAGS.includes(tagName)) {
    result = true;
